...
public class ConcurrentResourceService<T> {
	...
	T getFromCache(final String resId) {
		return (T) ServerCache.getObjectFromCache(cacheName, cachePrefix + resId);
	}

	void putInCache(final String resId, final T res) {
	    try {
	        ServerCache.addToCache(cacheName, cachePrefix + resId, res);
	...
	public T getSync(String resId) throws IIIFException {
		...
		try {
            ServerCache.addToCache(cacheName, resId, resT);
        } catch (IIIFException e) {
