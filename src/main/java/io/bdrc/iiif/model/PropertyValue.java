 ...
 *
 * <p>
 * Is organized as a mapping of languages to one or more values. See
 * http://iiif.io/api/presentation/2.1/#language-of-property-values and
 * http://iiif.io/api/presentation/2.1/#html-markup-in-property-values for more
 * information.
 */
...
public class PropertyValue {
    ...
    public void setValues(Locale language, String firstValue, String... rest) {
        ...
        // Need to wrap it with an ArrayList since we might want to add new values later
        // on
    ...
    /*
     * see http://iiif.io/api/presentation/2.1/#language-of-property-values example:
     * {"description": {"@value": "Here is a longer description of the object",
     * "@language": "en"}}
     *
     * In the case where multiple values are supplied, clients must use the
     * following algorithm to determine which values to display to the user.
     *
     * <ul> <li>If none of the values have a language associated with them, the
     * client must display all of the values.</li> <li>Else, the client should try
     * to determine the user’s language preferences, or failing that use some
     * default language preferences. Then:</li> <ul> <li>If any of the values have a
     * language associated with them, the client must display all of the values
     * associated with the language that best matches the language preference.</li>
     * <li>If all of the values have a language associated with them, and none match
     * the language preference, the client must select a language and display all of
     * the values associated with that language.</li> <li>If some of the values have
     * a language associated with them, but none match the language preference, the
     * client must display all of the values that do not have a language associated
     * with them.</li> </ul> </ul>
     */
