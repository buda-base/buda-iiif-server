...
public class SizeRequest {
     ...
     */
    @JsonCreator
    public static SizeRequest fromString(String str) throws IIIFException {
        if (str.equals("full")) {
    ...
    /**
     * Create a size request for the maximum supported size of the image region, if
     * isMax is true. If isMax is false, it behaves identically to the default
     * constructor.
     *
     * @param isMax true causes a size request for the maximum supported size of the
     *              image region
     */
    ...
    /**
     * Create a size request for a given width or height. One of both can be null
     * (the other value will be determined based on the aspect ratio of the image
     * region), but not both at once.
     *
     ...
     */
    public SizeRequest(Integer width, Integer height) throws IIIFException {
        if (width == null && height == null) {
            throw new IIIFException("Either width or height must be specified!");
        }
        this.width = width;
        this.height = height;
    }
    ...
    /**
     * Create a size request for a given width and height and signal that the server
     * can decide to render smaller resolutions as it deems neccessary.
     *
     * @param width   width of size request
     * @param height  height of size request
     * @param bestFit true, if server can decide to render smaller resolutions as it
     *                deems neccessary
     * @throws de.digitalcollections.iiif.model.image.ResolvingException if params
     *                                                                   can not be
     *                                                                   resolved to
     *                                                                   Size
     *                                                                   Request
     */
    public SizeRequest(int width, int height, boolean bestFit) throws IIIFException {
        this(width, height);
        this.bestFit = bestFit;
    }

    /**
     * Create a size request that scaled both dimensions according to a fixed
     * percentage, maintaining the aspect ratio.
     *
     ...
     */
    public SizeRequest(BigDecimal percentage) throws IIIFException {
        if (percentage.doubleValue() < 0 || percentage.doubleValue() > 100) {
            throw new IIIFException("Percentage must be between 0 and 100!");
        }
    ...
    /**
     * Return whether the server can decide to render smaller resolutions than
     * desired.
     *
     * @return true, if the server can decide to render smaller resolutions than
     *         desired
     */
     ...
     *
     * @see <a href="http://iiif.io/api/image/2.1/#canonical-uri-syntax">IIIF Image
     *      API specification</a>
     * @param nativeSize native size of request
     * @param profile    image api profile
     * @return canonical form of this request
     * @throws de.digitalcollections.iiif.model.image.ResolvingException if
     *                                                                   nativeSize
     *                                                                   can not be
     *                                                                   converted
     *                                                                   to
     ...
     *                                                                   form
     */
    public String getCanonicalForm(Dimension nativeSize, ImageApiProfile profile) throws IIIFException {
    ...
    public Dimension resolve(Dimension nativeSize, ImageApiProfile profile) throws IIIFException {
        return resolve(nativeSize, Collections.emptyList(), profile);
    }

    /**
     * Resolve the request to dimensions that can be used for scaling, based on the
     * native size of the image region and the available profile.
     *
     ...
     *                                                                   Dimension
     */
    public Dimension resolve(Dimension nativeSize, List<Dimension> availableSizes, ImageApiProfile profile) throws IIIFException {
        double aspect = (double) nativeSize.width / (double) nativeSize.height;
        // "max"
        if (max) {
            // By default, identical to the largest available size or the native size if no
            // sizes were
            // specified
            ...
            if (profile != null && profile.maxArea != null) {
                // Fit width and height into the maximum available area, preserving the aspect
                // ratio
                ...
                if (currentArea > profile.maxArea) {
                    ...
                    if (dim.width <= 0 || dim.height <= 0) {
                        throw new IIIFException(String.format("Cannot fit image with dimensions %dx%d into maximum area of %d pixels.",
        ...
        if (profile.maxWidth != null && out.width > profile.maxWidth) {
            throw new IIIFException(
                    String.format("Requested width (%d) exceeds maximum width (%d) as specified in the profile.", out.width, profile.maxWidth));
        } else if (maxHeight != null && out.height > maxHeight) {
            throw new IIIFException(
                    String.format("Requested height (%d) exceeds maximum height (%d) as specified in the profile.", out.height, maxHeight));
        } else if (profile.maxArea != null && out.height * out.width > profile.maxArea) {
            throw new IIIFException(String.format("Requested area (%d*%d = %d) exceeds maximum area (%d) as specified in the profile", out.width,
                    out.height, out.width * out.height, profile.maxArea));
            ...
            throw new IIIFException(
                    String.format("Requested dimensions (%dx%d) exceed native dimensions (%dx%d), profile states that upscaling is not supported.",
    ...
    /**
     * Like {@link #resolve(Dimension, ImageApiProfile)}, but can be used with a
     * {@link Rectangle}, e.g. as returned from
     * {@link RegionRequest#resolve(Dimension)}.
     *
     ...
     *                                                                   resolved
     */
    public Dimension resolve(Rectangle region, ImageApiProfile profile) throws IIIFException {
    ...
    public boolean equals(Object o) {
        ...
        return max == that.max && bestFit == that.bestFit && Objects.equal(width, that.width) && Objects.equal(height, that.height)
