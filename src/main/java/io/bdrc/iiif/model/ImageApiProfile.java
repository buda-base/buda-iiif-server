 ...
 *
 * <p>
 * Can be either a simple pre-defined profile (e.g.
 * {@link ImageApiProfile#LEVEL_ZERO}) or a complex profile describing the
 * available features of a given IIIF image.
 *
 * <p>
 * See http://iiif.io/api/image/2.1/#profile-description for an overview of all
 * available properties and features and
 * http://iiif.io/api/image/2.1/compliance/ for an overview of the available
 * compliance levels.
 */
public class ImageApiProfile extends Profile {

    public enum Format {
        JPG(MimeType.fromTypename("image/jpeg")), TIF(MimeType.fromTypename("image/tiff")), PNG(MimeType.fromTypename("image/png")),
        GIF(MimeType.fromTypename("image/gif")), JP2(MimeType.fromTypename("image/jp2")), PDF(MimeType.fromTypename("application/pdf")),
    ...
    public static class Feature {
        ...
        private enum ImageApiFeature {
            BASE_URI_REDIRECT, CANONICAL_LINK_HEADER, CORS, JSONLD_MEDIA_TYPE, MIRRORING, PROFILE_LINK_HEADER, REGION_BY_PCT, REGION_BY_PX,
            REGION_SQUARE, ROTATION_ARBITRARY, ROTATION_BY_90S, SIZE_ABOVE_FULL, SIZE_BY_WH_LISTED, SIZE_BY_FORCED_WH, SIZE_BY_H, SIZE_BY_PCT,
            SIZE_BY_W, SIZE_BY_WH, SIZE_BY_CONFINED_WH, SIZE_BY_DISTORTED_WH, OTHER
        }
        ...
        /**
         * A square region where the width and height are equal to the shorter dimension
         * of the complete image content. *
         */
        ...
        /**
         * Size of images may be requested in the form “w,h”, including sizes that would
         * distort the image. *
         */
        ...
        /**
         * Size of images may be requested in the form “w,h” where the supplied w and h
        ...
        public boolean equals(Object obj) {
            ...
            return Objects.equals(this.imageApiFeature, other.imageApiFeature) && Objects.equals(this.customFeature, other.customFeature);
        }
    }
    ...
    static {
        LEVEL_ONE.addFeature(Feature.REGION_BY_PX, Feature.SIZE_BY_W, Feature.SIZE_BY_H, Feature.SIZE_BY_PCT, Feature.BASE_URI_REDIRECT, Feature.CORS,
    ...
    static {
        LEVEL_TWO.addFeature(Feature.REGION_BY_PX, Feature.REGION_BY_PCT, Feature.SIZE_BY_W, Feature.SIZE_BY_H, Feature.SIZE_BY_PCT,
                Feature.SIZE_BY_CONFINED_WH, Feature.SIZE_BY_DISTORTED_WH, Feature.SIZE_BY_WH, Feature.ROTATION_BY_90S, Feature.BASE_URI_REDIRECT,
                Feature.CORS, Feature.JSONLD_MEDIA_TYPE);
        LEVEL_TWO.addFormat(Format.JPG, Format.PNG);
        LEVEL_TWO.addQuality(Quality.DEFAULT, Quality.COLOR, Quality.GRAY, Quality.BITONAL);
    }

    public static final Set<String> V1_PROFILES = ImmutableSet.of("http://library.stanford.edu/iiif/image-api/compliance.html#level0",
    ...
    /**
     * Merge multiple profiles into one. Useful for image servers that want to
     * consolidate the limits given in a info.json.
     *
     * @param profiles profiles to be merged
     * @return merged profile
     */
    public static ImageApiProfile merge(List<Profile> profiles) {
        return profiles.stream().filter(ImageApiProfile.class::isInstance).map(ImageApiProfile.class::cast).reduce(new ImageApiProfile(),
                ImageApiProfile::merge);
    }
