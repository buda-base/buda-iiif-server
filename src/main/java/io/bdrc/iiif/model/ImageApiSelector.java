...
public class ImageApiSelector {
    ...
    public static ImageApiSelector fromImageApiUri(URI imageApiUri) throws IIIFException {
        return fromString(imageApiUri.getPath());
    }

    public static ImageApiSelector fromString(String str) throws IIIFException {
        Matcher matcher = REQUEST_PAT.matcher(str);
        if (!matcher.find()) {
            throw new IIIFException("Malformed IIIF Image API request: " + str);
        }
        ...
        try {
            ...
            throw new IIIFException("Illegal value for quality: " + matcher.group("quality"));
        }
        try {
            selector.setFormat(Format.valueOf(matcher.group("format").toUpperCase()));
        } catch (IllegalArgumentException e) {
            throw new IIIFException("Illegal value for format: " + matcher.group("format"));
        }
        return selector;
    }
    ...
    public String toString() {
        return String.format("%s%s/%s/%s/%s.%s", identifier != null ? urlEncode(identifier) + "/" : "", Objects.toString(region, "full"),
                Objects.toString(size, "full"), Objects.toString(rotation, "0"), Objects.toString(quality, "default"),
                Objects.toString(format, "jpg"));
    }

    /**
     * The spec says we have to urlencode values, but only characters outside of the
     * US ASCII range and gen-delims from RFC3986. However, our URL library can only
     * encode the full set of gen-delims (EXCEPT the colon) and sub-delims, which
     * iswhy we have to manually decode the encoded sub-delims... Great and
     * pragmatic choice for readability, more code for us :-)
     */
     ...
     * @param defaultQuality The native/default quality of the image the selector is
     *                       applied to
     ...
     *                                                                   region
     */
    public String getCanonicalForm(Dimension nativeSize, ImageApiProfile profile, Quality defaultQuality) throws IIIFException {
        ...
        return String.format("%s%s/%s/%s/%s.%s", identifier != null ? urlEncode(identifier) + "/" : "", region.getCanonicalForm(nativeSize),
                size.getCanonicalForm(scaleReference, profile), rotation.toString(), quality.equals(defaultQuality) ? "default" : quality.toString(),
                format.toString());
    }
    ...
    public void setRegion(String region) throws IIIFException {
    ...
    public void setSize(String size) throws IIIFException {
    ...
    public void setRotation(String rotation) throws IIIFException {
