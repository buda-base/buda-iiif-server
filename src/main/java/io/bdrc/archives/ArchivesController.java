...
public class ArchivesController {
	...
	public ResponseEntity<String> getPdfLink(@PathVariable String id, @PathVariable String type,
		...
		case 6:
			...
			ResourceAccessValidation accValidation = new ResourceAccessValidation(
					(Access) request.getAttribute("access"), accessType);
	...
	public ResponseEntity<ByteArrayResource> downloadPdf(@PathVariable String name, @PathVariable String type)
		...
		ResponseEntity<ByteArrayResource> response = new ResponseEntity<ByteArrayResource>(new ByteArrayResource(array),
	...
	public ResponseEntity<String> jobState(@PathVariable String id, @PathVariable String type) throws Exception {
		Identifier idf = new Identifier(id, Identifier.MANIFEST_ID);
		String url = "download/file/" + type + "/" + idf.getVolumeId() + ":" + idf.getBPageNum() + "-"
				+ idf.getEPageNum() + "." + type;
		boolean done = false;
		if (type.equals(ArchiveBuilder.PDF_TYPE)) {
			done = ArchiveBuilder
					.isPdfDone(idf.getVolumeId() + ":" + idf.getBPageNum() + "-" + idf.getEPageNum() + ".pdf");
		}
		if (type.equals(ArchiveBuilder.ZIP_TYPE)) {
			done = ArchiveBuilder
					.isZipDone(idf.getVolumeId() + ":" + idf.getBPageNum() + "-" + idf.getEPageNum() + ".zip");
		}
	...
	public String getVolumeDownLoadLinks(ItemInfo item, Identifier idf, String type) throws BDRCAPIException {
		...
		for (VolumeInfoSmall vis : vlist) {
			...
			links = links + "<a type=\"application/" + type + "\" href=\"/download/" + type + "/v:"
					+ vis.getPrefixedId() + "::1-" + vi.totalPages + "\">Vol." + vis.getVolumeNumber() + " ("
					+ vi.totalPages + " pages) - " + vis.getPrefixedId() + "." + type + "</a><br/>";
		}
		return links;
	}

	public HashMap<String, HashMap<String, String>> getJsonVolumeLinks(ItemInfo item, String type)
	...
	private Iterator<String> getFairUseImgListIterator(int bPage, int ePage, VolumeInfo vi) {
		...
		ImageListIterator it2 = new ImageListIterator(vi.getImageList(), Integer.parseInt(vi.getTotalPages()) - 19,
				ePage);
		while (it2.hasNext()) {
			img.add(x, it2.next());
			x++;
